...
int main(int argc, char* argv[]) {
  ...
  //check if there is any user input for the matrices dimensions.
  if (argc > 1 && argc < 6) {
    m = atoi(argv[1]);
    n = atoi(argv[2]);
    p = atoi(argv[3]);
  ...
  // Allocate memory for the host(CPU) matrices
  h_A = (float*)malloc(m*n*sizeof(float));
  h_B = (float*)malloc(p*q*sizeof(float));
  h_C = (float*)malloc(m*q*sizeof(float));
 
  
  //initialize matrices on the host(CPU)
  ...
  //carry out host(CPU) matrix multiplication
  cpuMatmul(h_A,h_B,h_C,m,p,q);

  //End timer for host(CPU) Process
  t1 = clock();
  t1sum = ((double)(t1-t0))/CLOCKS_PER_SEC;
  printf("\nEntire CPU process took %f seconds.... Starting OpenAcc Process.\n", t1sum);
  ...
  //carry out openacc matrix multiplication on the device(GPU)
