...
program miniweather
  ...
  ! GPU_Data
  !******** These below variables are used in parallelizable regions
  !******** All are initialized on the host CPU but used on GPU
  !******** These variables are never modified after being initialized and describe static properties of domain
  !******** List of vars for copying: hy_dens_cell,hy_dens_theta_cell,hy_dens_int,hy_dens_theta_int,hy_pressure_int
  ...
  ! GPU_Data
  !******** These below variables are used in parallelizable regions
  !******** Both are initialized on the host CPU
  !******** Only "state" variable is needed for I/O in output() routine but not needed at program end otherwise
  !******** List of vars for copying: state,state_tmp
  ...
  ! GPU_Data
  !******** These below variables are used in parallelizable regions
  !******** They are only computed/instantiated inside these regions
  !******** List of vars for copying: flux,tend,sendbuf_l,sendbuf_r,recvbuf_l,recvbuf_r
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! TODO: MANAGE GPU DATA, see above "GPU_data" for hints
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
contains
      ...
      do k = 1 , nz
        do i = 1 , nx
          if (data_spec_int == DATA_SPEC_GRAVITY_WAVES) then
            ...
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            !! TODO: SPECIFY GPU ROUTINE OPTION
            !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine set_halo_values_x(state)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !! TODO: UPDATE DATA NEEDED FOR MPI
    !! TODO: ASYNC ME
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !! TODO: UPDATE DATA NEEDED FOR MPI
    !! TODO: ASYNC ME (IS A WAIT ALSO NEEDED HERE OR ABOVE?)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine output(state,etime)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !! TODO: UPDATE DATA FOR I/O FROM CPU
    !!
    !! NOTE: ONLY THE state VARIABLE IS NEEDED FROM THE GPU FOR THE I/O
    !!       THE hy_dens... VARIABLES ARE STATIC BETWEEN CPU/GPU
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
