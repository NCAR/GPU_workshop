...
program miniweather
  ...
  use openacc
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! TODO: Load NCCL library (only use when compiling for NVIDIA GPU)
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  #ifdef NV_GPU
    use nccl
  #endif
  ...
  #ifdef NV_GPU
    type(ncclDataType), parameter :: NCCL_RP = ncclDouble
  #endif
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !! Add vars for local MPI communicators to manage multi-GPU nodes
  integer  :: local_comm, local_rank              !Local comm and rank in MPI for multi-GPU nodes
  !! Add var to store number of GPUs per node
  integer  :: nGPUs_node
  !! TODO: Add vars for NCCL communicators (only use when compiling for NVIDIA GPU)
  #ifdef NV_GPU
...
contains
    ...
    do k = 1 , nz
      do i = 1 , nx+1
        ...
        do ll = 1 , NUM_VARS
          ...
          vals(ll) = (-stencil(1) + 7*stencil(2) + 7*stencil(3) - stencil(4))*(1.d0/12.d0)
  ...
  subroutine set_halo_values_x(state)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !! TODO: (1) Setup a non-blocking group section for calling NCCL communication tasks
    !!           See https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/api/group.html for API reference
    !!       (2) Insert NCCL versions of send and receive
    !!           Hint: For the cudaStream argument, use `acc_get_cuda_stream(acc_async_noval)`
    !!           Note: `acc_async_noval` => `!$acc ... async`    stream
    !!                 `N`               => `!$acc ... async(N)` stream
    !!           See https://docs.nvidia.com/deeplearning/nccl/user-guide/docs/api/p2p.html for API reference
    !!       (3) Specify the send/recv buffers to reference GPU device memory using OpenACC
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    #ifdef NV_GPU
        nccl_result = ncclGroupStart()
        !$acc host_data use_device (recvbuf_l,recvbuf_r, sendbuf_l,sendbuf_r)
        nccl_result = ncclRecv(recvbuf_l, hs*nz*NUM_VARS, NCCL_RP, left_rank,  nccl_comm, acc_get_cuda_stream(acc_async_noval))
        nccl_result = ncclSend(sendbuf_r, hs*nz*NUM_VARS, NCCL_RP, right_rank, nccl_comm, acc_get_cuda_stream(acc_async_noval))
        nccl_result = ncclRecv(recvbuf_r, hs*nz*NUM_VARS, NCCL_RP, right_rank, nccl_comm, acc_get_cuda_stream(acc_async_noval))
        nccl_result = ncclSend(sendbuf_l, hs*nz*NUM_VARS, NCCL_RP, left_rank,  nccl_comm, acc_get_cuda_stream(acc_async_noval))
        !$acc end host_data
        nccl_result = ncclGroupEnd()
    #else
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        !! CUDA aware MPI section
        !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        !Prepost receives
        !$acc host_data use_device (recvbuf_l,recvbuf_r)
        call mpi_irecv(recvbuf_l,hs*nz*NUM_VARS,MPI_RP, left_rank,0,MPI_COMM_WORLD,req_r(1),ierr)
        call mpi_irecv(recvbuf_r,hs*nz*NUM_VARS,MPI_RP,right_rank,1,MPI_COMM_WORLD,req_r(2),ierr)
        !$acc end host_data

        !$acc wait

        !Fire off the sends
        !$acc host_data use_device (sendbuf_l,sendbuf_r)
        call mpi_isend(sendbuf_l,hs*nz*NUM_VARS,MPI_RP, left_rank,1,MPI_COMM_WORLD,req_s(1),ierr)
        call mpi_isend(sendbuf_r,hs*nz*NUM_VARS,MPI_RP,right_rank,0,MPI_COMM_WORLD,req_s(2),ierr)
        !$acc end host_data
  ...
  subroutine init(dt)
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !! Set the GPU device for each MPI process per its *local_rank* on a node
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    #ifdef _OPENACC
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !! BEGIN NCCL SECTION
    !! TODO: (1) Generate unique NCCL id on rank 0
    !!       (2) Broadcast unique id to all processes
    !!       (3) Initialize the NCCL communicator
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    #ifdef NV_GPU
        if (myrank == 0) then
        ...
        call MPI_Bcast(nccl_id%internal, size(nccl_id%internal), MPI_CHAR, 0, MPI_COMM_WORLD, ierr)
  
        ! Initialize NCCL communicator
        nccl_result = ncclCommInitRank(nccl_comm, nranks, nccl_id, myrank)
    #endif
